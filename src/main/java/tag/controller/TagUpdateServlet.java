...
public class TagUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 회원정보 수정 처리용 컨트롤러
				...
				tag.setTagName(request.getParameter("update"));
				

				//3. 모델 서비스 메소드로 값 또는 객체 전달 실행하고 결과받기
				int result = new TagService().updateTag(tag);
				...
				if(result > 0) {
					...
					request.setAttribute("message", tag.getTagName() + " 태그네임 수정 실패!");
