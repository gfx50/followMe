 ...
 */
@WebServlet(asyncSupported = true, urlPatterns = { "/areg" })
public class AdminRegistServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 회원가입 처리용 컨트롤러
				...
				Admin Admin = new Admin();  //모델 서비스로 보낼 값의 갯수가 1개 이상이면 객체에 저장함
				...
				//?
//				member.setBirthDate(request.getParameter(""));
...
//				
				...
				int result = new AdminService().registAdmin(Admin);
				//에러 페이지 테스트
				//int result = 0;
				...
				if(result > 0) {
					...
					response.sendRedirect("/fm/views/admin/adminLogin.jsp");
				}else {
