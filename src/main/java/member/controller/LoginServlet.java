...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("login : " + useremail + ", " + userpwd);
		 ...
		 * 
		 * //패스워드 문자열을 암호문으로 바꾸려면, byte[] 로 변환해야 함 byte[] pwdValues =
		 * userpwd.getBytes(Charset.forName("UTF-8")); //암호문으로 바꾸기 md.update(pwdValues);
		 * //암호화된 byte[] 을 String 으로 바꿈 : 암호문 상태임 cryptoUserpwd =
		 * Base64.getEncoder().encodeToString(pwdValues);
		 * 
		 * //확인 //System.out.println("암호화된 패스워드 : " + cryptoUserpwd);
		 * //System.out.println("글자길이 : " + cryptoUserpwd.length()); } catch (Exception
		 * e) { e.printStackTrace(); }
		 */
		
		//3. 서비스 메소드로 값 전달 실행하고 결과받기
		Member loginMember = new MemberService().selectLogin(useremail, userpwd);
