 ...
 */
@WebServlet("/settagid")
public class TagIdUpdateServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TagIdUpdateServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 회원정보 수정 처리용 컨트롤러
				...
				tagid.setTagName(request.getParameter("update"));
				

				//3. 모델 서비스 메소드로 값 또는 객체 전달 실행하고 결과받기
				int result = new TagIdService().updateTagId(tagid);
				...
				if(result > 0) {
					...
					request.setAttribute("message", tagid.getTagName() + " 태그네임 수정 실패!");
