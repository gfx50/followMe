...
/**
 * Servlet implementation class TrailReportServlet
 */
@WebServlet("/treport")
public class TrailReportServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TrailReportServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 게시글 신고 처리용 컨트롤러
			...
			trail.setTrailReport(request.getParameter("trailreport"));
			
			//3. 모델 서비스 메소드로 값 또는 객체 전달 실행하고 결과받기
			int result = new TrailService().updateTrailReport(trail, trailId);
		
			//4. 받은 결과로 내보낼 뷰 선택 처리
			if(result > 0) {
				//수정 성공시 마이 페이지 뷰가 출력되게 한다면, main 서블릿을 구동시킴
				//서블릿에서 다른 서블릿을 실행시킬 수 있음
				response.sendRedirect("/fm/tdetail?trailId=" + trailId);
