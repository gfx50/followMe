...
/**
 * Servlet implementation class TrailFollowListServlet
 */
@WebServlet("/tflist")
public class TrailFollowListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TrailFollowListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 페이지별로 출력되는 게시글 목록 조회 요청 처리용 컨트롤러
				...
				System.out.println("trailfollowlistServlet : " +mid);
				//총 페이지 수 계산을 위한 전체 목록 갯수 조회
				int listCount = tservice.getFollowListCount(mid);
				//System.out.println("blist : " + listCount);
				System.out.println("trailfollowlistServlet listCount : " + listCount);
				
				//뷰 페이지에서 사용할 페이징 관련 값 계산 처리
				Paging paging = new Paging(listCount, currentPage, limit, "tflist");
				paging.calculator();
		
	   
		// 팔로우 게시물 최근순으로 불러오기
		ArrayList<Trail> list = tservice.selectFollowerList(mid, paging.getStartRow(), paging.getEndRow());
